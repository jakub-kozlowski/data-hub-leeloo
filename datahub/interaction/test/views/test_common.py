...
class TestGetInteraction(APITestMixin):
    ...
    def test_adviser_and_team_returned_from_participants(self):
        """
        Test that dit_adviser and dit_team are returned from dit_participants and not the
        dit_adviser and dit_team model fields.
        """
        interaction = CompanyInteractionFactory()

        interaction.dit_adviser = None
        interaction.dit_team = None
        interaction.save()
        ...
        response_data = response.json()
        assert response_data['dit_adviser'] == {
